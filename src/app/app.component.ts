...
import { MsgExecuteContract, MsgExecuteContractParams, MsgSnip721AddMinter, Permit, QueryContractRequest, SecretNetworkClient, ViewingKey } from "secretjs";
...
export class AppComponent implements AfterViewInit {
  ...
  getData() {
    this.window.keplr?.experimentalSuggestChain({
      chainId: "secretdev-1",
    ...
    this.window.keplr?.enable('secretdev-1').then(() => {
        ...
        offlineSigner.getAccounts().then((account: any) => {
          ...
          secretjs.then((secretjs) => {
            ...
            secretjs.query.snip721.contractCodeHash('secret174kgn5rtw4kf6f938wm7kwh70h2v4vcfft5mqy').then((hash) => {
              ...
              secretjs.tx.snip721.mint(msg).then((v) => { console.log(v)})
            })
  ...
  async sign() {
    ...
    SecretNetworkClient.create({
      ...
      if (localStorage.getItem('permit')) {
        this.getToken(secretjs, JSON.parse(localStorage.getItem('permit') || ''))
      } else {
        ...
        //   console.log(v);
        // })
        secretjs.utils.accessControl.permit.sign(this.yourAddress, 'secretdev-1', 'one-net-auth', ["secret174kgn5rtw4kf6f938wm7kwh70h2v4vcfft5mqy"], ['owner'], true).then((_permit: Permit) => {
  ...
  getToken(secretjs: SecretNetworkClient, permit: Permit) {
    secretjs.query.snip721.contractCodeHash('secret174kgn5rtw4kf6f938wm7kwh70h2v4vcfft5mqy').then((res) => {
      let contract: SecretContract = {
      ...
      secretjs.query.snip721.GetOwnedTokens(ownedTokenReq).then((v) => {
        ...
        let tokenInfoReq: {contract: SecretContract, auth: Auth, token_id: string } = {
          contract: contract,
          auth: {permit: permit},
